...
//More information on the In/OutModule structures can be found in the Winamp input plug-in SDK
typedef struct OutModule
{
	s32	version;
	s8	*description;
	s32	id;
	...
	v0	(*Config)(HWND hwndParent);
	v0	(*About)(HWND hwndParent);
	v0	(*Init)();
	v0	(*Quit)();
	s32	(*Open)(s32 sampleRate, s32 numChannels, s32 bitsPerSmp, s32 bufferLenms, s32 preBufferms);
	v0	(*Close)();
	s32	(*Write)(s8 *buf, s32 len);
	s32	(*CanWrite)();
	s32	(*IsPlaying)();
	s32	(*Pause)(s32 pause);
	v0	(*SetVolume)(s32 volume);
	v0	(*SetPan)(s32 pan);
	v0	(*Flush)(s32 t);
	s32	(*GetOutputTime)();
	s32	(*GetWrittenTime)();
...
typedef struct InModule
{
	...
	s32	seekable;
	s32	usesOutput;
	v0	(*Config)(HWND hParent);
	v0	(*About)(HWND hParent);
	v0	(*Init)();
	v0	(*Quit)();
	v0	(*GetFileInfo)(s8 *file, s8 *title, s32 &length);
	s32	(*InfoBox)(s8 *file, HWND hParent);
	s32	(*IsOurFile)(s8 *file);
	s32	(*Play)(s8 *file);
	v0	(*Pause)();
	v0	(*UnPause)();
	s32	(*IsPaused)();
	...
	v0	(*SetVolume)(s32 volume);
	v0	(*SetPan)(s32 pan);
	v0	(*SAVSAInit)(s32 maxLatency, s32 smpRate);
	v0	(*SAVSADeInit)();
	v0	(*SAAddPCMData)(v0 *pcmData, s32 numChn, s32 bps, s32 timestamp);
	s32	(*SAGetMode)();
	v0	(*SAAdd)(v0 *data, s32 timestamp, s32 csa);
	v0	(*VSAAddPCMData)(v0 *pcmData, s32 numChn, s32 bps, s32 timestamp);
	s32	(*VSAGetMode)(s32 *specNCh, s32 *waveNCh);
	v0	(*VSAAdd)(v0 *data, s32 timestamp);
	v0	(*VSASetInfo)(s32 numChn, s32 smpRate);
	s32	(*DSPisActive)();
	s32	(*DSPdoSamples)(s16 *samples, s32 numSmp, s32 bps, s32 numChn, s32 rate);
	v0	(*EQSet)(s32 on, s8 data[10], s32 preamp);
	v0	(*SetInfo)(s32 bitrate, s32 rate, s32 stereo, s32 synched);
	OutModule	*outMod;
	HWND		hMainWndNew;
} InModule;       

extern InModule	inMod;
...
//
// Used to change the DSP options during emulation
//
// In:
//    reset = true, if the output plug-in needs to be reopened

v0	__fastcall ChangeOutput(b8 reset);


//**************************************************************************************************
// Mute Voice
//
// Sets the mute state of a voice in the DSP
//
// In:
//    voice = Voice to set state of (0-7)
//    state = State of voice (true = muted)
...
// Set Amplification
//
// Sets the amplification level in the DSP and updates any visible controls
//
// In:
//    amp = Amplification level (32768-524288, -1 resets controls with current level)

v0	__fastcall SetAmp(u32 amp);


//**************************************************************************************************
// Set ID666 Tag
//
// Updates the current song's ID666 information, if the song playing is the same as the filename
// passed in.
//
// In:
//    id6 -> ID666 object containing update information
...
//
// Sets a flag that the timer should be enabled.  By specifying a bit, multiple controls can enable
// or disable the timer.  Only when all controls are set to enabled will the timer work.
//
// In:
//    bit   = Bit to use (0-31)
//    state = Enabled
...
// Get Output Time
//
// Returns the time (in ticks) output by the output plug-in
...
#define	WM_WA_MPEG_EOF	WM_USER+2           //Message sent to Winamp after the music has stopped.
